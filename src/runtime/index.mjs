...
export async function command(binary, args, options = {}) {
  try {
    ...
    const env = { ...environment, PATH: dirname(process.execPath) + delimiter + (environment.PATH || '') };
    const { stdout } = await exec(binary, args, { timeout: 30000, maxBuffer: 24 * 1024 * 1024, ...options, env });
    return stdout;
  } catch (error) {
