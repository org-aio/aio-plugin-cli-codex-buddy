...
统一入口为 `codex-buddy router`。每轮从当前 Codex 供应商读取全量 `/v1/models`，不按供应商或家族白名单过滤。
...
复杂父任务通过 `planning` 功能选择强模型规划者及经济型执行候选；执行候选只用于拆分后满足能力要求的子任务，不代表已经创建或启用。简单/常规父任务沿用直接路由。
